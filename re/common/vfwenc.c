/* vfwenc.c - BATCH VFW DIV3 encoder. Reads N concatenated grayscale frames (each W*H
   bytes), compresses each as an independent DIV3 keyframe with the DivX 3.11 / mpg4c32
...
int main(int argc, char** argv){
    ...
    HIC hic=ICOpen(ICTYPE_VIDEO, mmioFOURCC('D','I','V','3'), ICMODE_COMPRESS);
    if(!hic){ fprintf(stderr,"ICOpen DIV3 fail\n"); return 1; }
    ...
    for(int fr=0; fr<NF; fr++){
        ...
        obi->biSizeImage=0;
        DWORD r=ICCompress(hic, ICCOMPRESS_KEYFRAME, obi, outbuf, &bi, rgb,
